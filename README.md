...
- **The prompts are the surface you iterate on so make that loop fast.** There are two, tuned independently — `prompts/briefing.md` (pick the day's signal) and `prompts/discover.md` (rank accounts worth following). An [offline eval harness](./tests/eval-briefing.test.ts) runs the actual briefing selection over a frozen batch of posts with no Discord post or D1 write, so you can A/B prompt edits against identical input. Version the prompts, not just the code.
